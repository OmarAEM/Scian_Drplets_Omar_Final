...
#        showing(edges,"Edge detection Image")
        ...
        #Guardo informacion de la imagen junto a su tension superficial
        total_folder = 0
        dir = folder+"/Resultados/"
        for path in os.listdir(dir):
            if os.path.isfile(os.path.join(dir, path)):
                total_folder += 1
        save_Gamma=folder+"/Resultados/"+str(total_folder+1)+"_Surface_Tension_"+str(round(GammaValues[0],4))+".png"
                
        cv2.imwrite(save_Gamma,image)  
